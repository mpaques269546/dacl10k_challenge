...
from .detr_transformer import (DetrTransformerDecoder, DetrTransformerDecoderLayer,DetrTransformerEncoder, DetrTransformerEncoderLayer)
...
class DeformableDetrTransformerEncoder(DetrTransformerEncoder):
    ...
    def __init__(num_layers: int=6,
        ...
        super(DetrTransformerEncoder).__init__(num_layers=num_layers, self_attn_cfg=self_attn_cfg,ffn_cfg =ffn_cfg, norm_cfg=norm_cfg)
    """
    def _init_layers(self) -> None:
        """Initialize encoder layers."""
        self.layers = nn.ModuleList([
            DeformableDetrTransformerEncoderLayer(self_attn_cfg= self.self_attn_cfg,
                 ffn_cfg =self.ffn_cfg,norm_cfg =  self.norm_cfg) # **self.layer_cfg
...
class DeformableDetrTransformerDecoder(DetrTransformerDecoder):
    ...
    def _init_layers(self) -> None:
        ...
        self.layers = nn.ModuleList([
            DeformableDetrTransformerDecoderLayer(self_attn_cfg =self.self_attn_cfg,cross_attn_cfg =self.cross_attn_cfg,
                 ffn_cfg  = self.ffn_cfg,
                 norm_cfg = self.norm_cfg) # **self.layer_cfg
            for _ in range(self.num_layers)
        ])
        self.embed_dims = self.layers[0].embed_dims
...
class DeformableDetrTransformerDecoderLayer(DetrTransformerDecoderLayer):
    ...
    def _init_layers(self) -> None:
        ...
        self.cross_attn = MultiScaleDeformableAttention(**self.cross_attn_cfg) # self.cross_attn -> self.attentions, , num_levels=3
        self.embed_dims = self.self_attn.embed_dims
        self.ffns = FFN(**self.ffn_cfg)
        norms_list = [
            build_norm_layer(self.norm_cfg, self.embed_dims)[1]
