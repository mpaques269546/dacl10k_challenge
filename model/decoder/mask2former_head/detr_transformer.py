...
class DetrTransformerEncoder(nn.Module):
    ...
    def _init_layers(self) -> None:
        ...
        self.layers = nn.ModuleList([
            DetrTransformerEncoderLayer(self_attn_cfg= self.self_attn_cfg,
...
model = DetrTransformerEncoder(num_layers=8)
...
class DetrTransformerDecoder(nn.Module):
    ...
    def __init__(self,
                 ...
                 return_intermediate: bool = True,) -> None:
        super().__init__()
        self.num_layers = num_layers
        self.self_attn_cfg = self_attn_cfg 
    ...
    def _init_layers(self) -> None:
        ...
        self.layers = nn.ModuleList([
            DetrTransformerDecoderLayer(self_attn_cfg = self.self_attn_cfg,
...
class DetrTransformerDecoderLayer(nn.Module):
    ...
    def _init_layers(self) -> None:
        """Initialize self-attention, FFN, and normalization."""
        self.self_attn = MultiheadAttention(**self.self_attn_cfg)
        self.cross_attn = MultiheadAttention(**self.cross_attn_cfg) # self.cross_attn -> self.attentions
        self.embed_dims = self.self_attn.embed_dims
        self.ffns = FFN(**self.ffn_cfg)
        norms_list = [
            build_norm_layer(self.norm_cfg, self.embed_dims)[1]
