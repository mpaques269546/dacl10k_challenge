...
from .utils import caffe2_xavier_init, MlvlPointGenerator, SinePositionalEncoding
from .transformer_decoder import Mask2FormerTransformerEncoder
...
class MSDeformAttnPixelDecoder(nn.Module):
    ...
    def __init__(self,
                 ...
                 num_layers:int=6,
                 num_encoder_levels:int=4, 
                 self_attn_cfg= dict(embed_dims=256, num_levels=3, num_heads=8, dropout=0.0), 
                 ffn_cfg = dict(embed_dims=256, feedforward_channels=1024, num_fcs=2,  ffn_drop=0.,act_cfg=dict(type='ReLU', inplace=True)),
                ) -> None:
        super().__init__()
        self.encoder = Mask2FormerTransformerEncoder(num_layers=num_layers, self_attn_cfg=self_attn_cfg, ffn_cfg =ffn_cfg) 
        # Mask2FormerTransformerEncoder(num_layers=6, self_attn_cfg= dict(embed_dims=256, num_heads=8, dropout=0.0), ffn_cfg = dict(embed_dims=256, 
        #feedforward_channels=1024, num_fcs=2,  ffn_drop=0.,act_cfg=dict(type='ReLU', inplace=True)),norm_cfg = dict(type='LN'))
        self.positional_encoding = SinePositionalEncoding(**positional_encoding) # SinePositionalEncoding( num_feats= 384, temperature= 10000, normalize = False,scale= 2 * math.pi, eps= 1e-6, offset = 0.,)
        
        self.strides = strides
        self.num_input_levels = len(in_channels)
        self.num_encoder_levels = num_encoder_levels # self.encoder.layer_cfg.self_attn_cfg.num_levels
    ...
    def init_weights(self) -> None:
        ...
        for i in range(0, self.num_encoder_levels):
            ...
            torch.nn.init.xavier_uniform_(self.input_convs[i].conv.weight, gain=1.0)
        ...
        torch.nn.init.normal_(self.level_encoding.weight , mean=0.0, std=1.0)
