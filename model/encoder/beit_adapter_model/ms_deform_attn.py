...
class MSDeformAttnFunction(Function):
    ...
    def forward(ctx, value, value_spatial_shapes, value_level_start_index,
        ...
        output = MSDA.forward(ctx, value=value, value_spatial_shapes=value_spatial_shapes, value_level_start_index=value_level_start_index, 
             sampling_locations=sampling_locations, attention_weights=attention_weights, im2col_step= ctx.im2col_step)

        # MDSA init:   embed_dims: int = 256, num_heads: int = 8, num_levels: int = 4,
        # num_points: int = 4, im2col_step: int = 64,dropout: float = 0.1,
        #batch_first: bool = False,
        #         norm_cfg: Optional[dict] = None,
        #         init_cfg: Optional[mmcv.ConfigDict] = None
        #mdsa = MultiScaleDeformableAttention(embed_dims=256, num_heads=8, num_levels= 4,
        #num_points = 4, im2col_step = 64,dropout= 0.1, batch_first= True,)
        ...
        #        level_start_index:
        #output = mdsa.forward(value, value_spatial_shapes,
...
class MSDeformAttn(nn.Module):
    ...
    def forward(self, query, reference_points, input_flatten, input_spatial_shapes,
        ...
        #output = MSDA.apply(value, input_spatial_shapes, input_level_start_index,
        #sampling_locations, attention_weights, self.im2col_step)
        output = ms_deform_attn_core_pytorch(value, 
            input_spatial_shapes,sampling_locations, attention_weights)
